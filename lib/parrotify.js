...
class Parrotify extends Component {
	...
	render(props, state) {
		if ( !this.state.parrot ) {
			...
			return <div><Spinner green/> Busy generating your parrot</div>
		}
		return <Image preserveAspectRatio src={`${this.state.parrot}`}/>;
	}

	componentDidMount() {
		const self = this;
		got(`${this.basicCall}`, { encoding: null })
		.then(response => {
...
const unmount = render(<Parrotify/>);
